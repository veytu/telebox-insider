...
class MaxTitleBar extends DefaultTitleBar {
  ...
  render() {
    ...
    this.sideEffect.addDisposer([
      ...
      valueEnhancer.combine([this.state$, this.boxes$, this.normalBoxes$]).subscribe(([state, titles]) => {
        if (state === TELE_BOX_STATE.Maximized) {
...
class TeleBoxManager {
  constructor({
    ...
    this.normalBoxes = new valueEnhancer.Val(normalBoxes);
  ...
  create(config = {}, smartPosition = true) {
    ...
    const managerMaximized$ = valueEnhancer.combine([this._maximized$, this.normalBoxes], ([maximized, normalBoxes]) => {
      return maximized && !normalBoxes.includes(id);
    });
    const managerMinimized$ = valueEnhancer.combine([this._minimized$, this.normalBoxes], ([minimized, normalBoxes]) => minimized && !normalBoxes.includes(id));
