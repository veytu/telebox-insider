...
class TeleBox {
  constructor({
    ...
    const maximized$ = valueEnhancer.combine([boxMaximized$, managerMaximized$], ([boxMaximized, managerMaximized]) => {
    ...
    const state$ = valueEnhancer.combine([minimized$, maximized$], ([minimized, maximized]) => {
...
class TeleBoxCollector {
  constructor({
    ...
    const visible$ = valueEnhancer.derive(minimizedBoxes$, (minimizedBoxes) => minimizedBoxes.length > 0);
    ...
    const popupVisible$ = new valueEnhancer.Val(false);
    const valConfig = {
      styles: styles$,
      $collector: el$
    };
...
class TeleBoxManager {
  constructor({
    ...
    const maximizedBoxes$ = new valueEnhancer.Val(maximizedBoxes);
    const minimizedBoxes$ = new valueEnhancer.Val(minimizedBoxes);
    if (root$.value) {
    ...
    const state$ = valueEnhancer.combine([minimizedBoxes$, maximizedBoxes$], ([minimized, maximized]) => minimized.length ? TELE_BOX_STATE.Minimized : maximized.length ? TELE_BOX_STATE.Maximized : TELE_BOX_STATE.Normal);
  ...
  create(config = {}, smartPosition = true) {
    const id = config.id || sideEffectManager.genUID();
    const managerMaximized$ = new valueEnhancer.Val(this._maximizedBoxes$.value.includes(id));
    const managerMinimized$ = new valueEnhancer.Val(this._minimizedBoxes$.value.includes(id));
