...
class MaxTitleBar extends DefaultTitleBar {
  ...
  updateTitles() {
    ...
    Object.entries(this.allBoxStatusInfo).filter(([_, state]) => state === TELE_BOX_STATE.Minimized).map(([boxId, _]) => boxId);
...
class TeleBoxManager {
  constructor({
    ...
    this.lastLastNotMinimizedBoxsStatus$.reaction(
      (lastLastNotMinimizedBoxsStatus2, _, skipUpdate) => {
        if (!skipUpdate) {
  ...
  getNomalBoxes(allBoxStatusInfo = this.allBoxStatusInfo$.value) {
    ...
    return Object.entries(allBoxStatusInfo).filter(([_, state]) => state === TELE_BOX_STATE.Normal).map(([boxId, _]) => boxId);
  }
  ...
  create(config = {}, smartPosition = true) {
    ...
    if (!((_a = this.allBoxStatusInfo$.value) == null ? void 0 : _a[box.id])) {
  ...
  makeBoxTopFromMaximized(boxId) {
    ...
    if (boxId) {
      ...
      const allBoxStatusInfo = this.allBoxStatusInfo$.value || {};
      const maximizedBoxes = (_a = this.boxes) == null ? void 0 : _a.filter(
        (box) => allBoxStatusInfo[box.id] === TELE_BOX_STATE.Maximized
      );
